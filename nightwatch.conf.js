// const dotenv = require('dotenv'); // optionally store youre Evironment Variables in .env
...
/*
  Selenium-download does exactly what it's name suggests; downloads (or updates) the version of
  Selenium (& chromedriver) on your localhost where it will be used by Nightwatch. The following
  code checks for the existence of `selenium.jar` before trying to run our tests. This should later
  be expanded to to run an update using the api, but for now this will do.
 */
